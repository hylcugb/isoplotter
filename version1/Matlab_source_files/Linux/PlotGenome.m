...
% In Windows (32bit): tic; mcc -m -I 'D:\My Documents\Programming\MatLab\Work\' -d 'D:\Temp\' PlotGenome; toc
...
function PlotGenome(input_file, output_file)
    ...
    for j=1:3
        ...
        %Save figure
        filename = max(strfind(output_file, '.'));
        if isempty(filename)
        ...
        disp(['Saving plot to: ' curr_output]); 
        %set(gcf, 'paperpositionmode', 'auto');
        %set(gcf, 'Renderer', 'zbuffer');
        %saveas(gcf, curr_output);
        %print curr_output -dtiff -painters;
        print('-depsc','-tiff','-r300',curr_output)
        disp(['The plot was saved.']);
        close all;
    end;
